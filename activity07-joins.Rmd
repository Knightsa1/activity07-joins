...
These data were scrapped from [Denny's](sta) Roundel [La Quinta's](https://www.wyndhamhotels.com/laquinta/locations) location web pages by Mine Çetinkaya-Rundel.
...
**Response**: There are La Quinta's outside of the US, Denny's is only in the US.
...
**Response**: We would need to look at the longitude and latitude and see if they both fall within the US.
...
**Response**:
Delaware has the fewest Denny's locations with one.
...
**Response**:
Technically Delaware and Hawaii have no La Quintas, but Maine has only one.


Is this surprising to you? Why or why not?

**Response**: It is not surprising that a small state has only one Denny's, but no La Quinta's. I think what is more surprising is that there is a Denny's in Hawaii.
...
```{r dennys_per_state}
dennys_per_state <- dennys %>% 
  group_by(state) %>% 
  count(state, sort = TRUE)

```

```{r joining_dennys_with_states}

dennys_state <- dennys_per_state %>% 
...
dennys_state %>% 
  mutate(dennys_per_1000mi = n / area) %>% 
  arrange(desc(dennys_per_1000mi))
```

Which states have the most Denny's locations per thousand square miles?

**Response**: DC or Rhode Island have the most Denny's location per 1000 miles.
...
```{r laquinta_per_state}

laquinta_per_state <- laquinta_us %>% 
  group_by(state) %>% 
  count(state, sort = TRUE)

```

```{r joining_laquinta_with_states}

laquinta_states <- laquinta_per_state %>% 
  inner_join(states, by = c("state" = "abbreviation"))

```

```{r laquinta_per_1000mi}
laquinta_states %>% 
  mutate(laquinta_per_1000mi = n / area) %>% 
  arrange(desc(laquinta_per_1000mi))
```

**Response**: Rhode Island has the moste La Quintas per 1000 square miles.
